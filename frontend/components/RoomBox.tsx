...
export const RoomBox = ({
  ...
  return (
    ...
    >
      <div className="w-full flex flex-row items-center justify-center" style={{ gap: isAvailable ? '0' : '5px' }}>
        <h1 style={{ color: isAvailable ? roomsubColor : 'black' }}>Room</h1>
        <div className='bg-gray-500 text-white px-2 rounded-sm' style={{ paddingLeft: isAvailable ? '0px' : '8px', paddingRight: isAvailable ? '0px' : '8px' }} >
          {isAvailable ? '' : 'Booked'}
        </div>
      </div>
      ...
      <h1 style={{ color: isAvailable ? roomsubColor : 'black', fontWeight: 'bold' }}>
        {roomType}
      </h1>
    </Button>
  )
